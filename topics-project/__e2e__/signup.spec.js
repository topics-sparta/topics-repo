...
test("sign up new user", async ({ page }) => {
  ...
  expect(await page.getByPlaceholder("Enter your name").inputValue()).toBe(
    "John Doe"
  );
  expect(await page.getByPlaceholder("Enter your email").inputValue()).toBe(
    randomEmail
  );
  expect(await page.getByPlaceholder("Create a password").inputValue()).toBe(
    "test123"
  );
  expect(await page.getByPlaceholder("Confirm password").inputValue()).toBe(
    "test123"
  );
